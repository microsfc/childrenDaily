...
class _CalendarEventPageState extends State<CalendarEventPage> {
  ...
  List<NeatCleanCalendarEvent> _neatEvents =
      []; // flutter_neat_and_clean_calendar 的事件格式
  ...
  Future<CalendarEvent?> _showAddEventDialog() async {
    ...
    ValueNotifier<TimeOfDay> inputStartTime =
        ValueNotifier(TimeOfDay.fromDateTime(startTime));
    ValueNotifier<TimeOfDay> inputEndTime =
        ValueNotifier(TimeOfDay.fromDateTime(endTime));
                  ...
                  StreamBuilder(
                      ...
                      builder: (context, snapshot) {
                        ...
                        for (var doc in snapshot.data!.docs) {
                          ...
                          if (userId ==
                              Provider.of<CalendarService>(context,
                                      listen: false)
                                  .getCurrentUserId()) {
                            continue;
                          }
                          ...
                          sharedUserList.add(StatefulBuilder(builder:
...
// 顯示編輯事件對話框
  Future<CalendarEvent?> _showEditEventDialog(CalendarEvent eventToEdit) async {
    ...
    ValueNotifier<DateTime> startTimeNotifier =
        ValueNotifier(eventToEdit.startTime);
    ValueNotifier<DateTime> endTimeNotifier =
        ValueNotifier(eventToEdit.endTime);
                    ...
                    IconButton(
                      ...
                      onPressed: () async {
                        final TimeOfDay? pickedTime = await showTimePicker(
                          context: context,
                          initialTime:
                              TimeOfDay.fromDateTime(startTimeNotifier.value),
                        );
                        if (pickedTime != null) {
                          setState(() {
                    ...
                    IconButton(
                      ...
                      onPressed: () async {
                        final TimeOfDay? pickedTime = await showTimePicker(
                          context: context,
                          initialTime:
                              TimeOfDay.fromDateTime(endTimeNotifier.value),
                        );
                        if (pickedTime != null) {
                          setState(() {
    ...
    if (event != null) {
      ...
      if (newEvent != null) {
        ...
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('新增事件成功')));
      } else {
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('新增事件失敗')));
      }
    }
  }
  ...
  void editEvent(CalendarEvent event) async {
    ...
    if (updatedEvent != null) {
      final CalendarEvent? updateEvent =
          await _calendarService!.updateEvent(updatedEvent);
      if (updateEvent != null) {
        _buildEventList(_selectedDay);
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('編輯事件成功')));
      } else {
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('編輯事件失敗')));
      }
    }
  }
  ...
  void deleteEvent(CalendarEvent event) async {
    ...
    if (confirmDelete) {
      ...
      if (success) {
        _buildEventList(_selectedDay);
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('刪除事件成功')));
      } else {
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('刪除事件失敗')));
      }
    }
  }
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                  ...
                  subtitle: Text(
                      '${DateFormat('yyyy-MM-dd HH:mm').format(event.startTime)} - ${DateFormat('yyyy-MM-dd HH:mm').format(event.endTime)}'),
                  trailing: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      IconButton(
  ...
  List<NeatCleanCalendarEvent> _buildNeatCleanCalendarEventsMap(
  ...
  Widget _buildNeatCleanCalendarEventList(
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          ...
          title: Text(
              event.summary), // 使用 summary (即 NeatCleanCalendarEvent 的事件標題)
          subtitle: Text(
