...
class CalendarService {
  ...
  Future<List<CalendarEvent>> getAllEventsList() async {
   final snapshot = await _firestore
        .collection(EVENT_COLLECTION)
        .where(
          'sharedWith', arrayContains: getCurrentUserId())
        .orderBy('startTime')
        .get();
    return snapshot.docs.map((doc) => CalendarEvent.fromeFirestore(doc.data(), doc.id)).toList();
  }
